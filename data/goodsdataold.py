...
class GoodsData:
	...
	def recommend_one(self, model):
		...
		for data in post_data:
			...
			choice = random.choice(recommend_list['data'])
			if not choice:
				continue
	...
	def spec(self, model):
		...
		for data in post_data:
			...
			for type_data in order_type:
				goods_list = self.get_shopping(type_data, [2])
				if not goods_list:
					continue
		...
		for data in post_data:
			for type_data in [1, 2, 3]:
				for goods_type in [1, 2, 3]:
					goods_list = self.get_shopping(type_data, [goods_type])
					if not goods_list:
						continue
	...
	def package(self, model):
		...
		for data in post_data:
			...
			order_type = [1, 2, 3]
			for type_data in order_type:
				goods_list = self.get_shopping(type_data, [3])
				if not goods_list:
					continue

				choice = random.choice(goods_list)
				data['glid'] = choice['id']
				data['type'] = type_data
	...
	# 点餐先行条件
	def get_shopping(self, shop_type = 1, goods_type = [1], shop_reserve_data = 1, send_time = ""):
		# # shop_type 1堂食 2外带 3外卖 goods_type 1单品 2多规格 3套餐
		# config = self.get_config_data.get_conf("getCateListUrl")
		# # 获取分类
		...
		for result_data in params['data']['list']:
			...
			for goods_data in goods_list['data']:
				if goods_data['id'] in goods_id:  # 去重
					continue
				# if goods_data['isSale'] == 2:  # 去掉不可选择的（非预定单）
				# 	continue
				...
				if goods_data['type'] in goods_type:
					goods_data['detail'] = {}
					if goods_data['type'] == 2:
						# goods_detail = config['host'] + "/goods/info/spec"
						# goods_detail_post = {"glid": goods_data['id']}
						# goods_spec_data = self.send_post.send_post(goods_detail, goods_detail_post, header)
						goods_spec_data = self.get_spec(['下单', '规格详情'], [{"glid": goods_data['id']}])
						goods_data['detail'] = goods_spec_data['data']
					elif goods_data['type'] == 3:
